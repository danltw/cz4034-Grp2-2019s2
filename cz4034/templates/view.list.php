  ...
  <ul class="no-bullet">
    ...
    foreach ($results->response->docs as $doc):
      ...
      $id = $doc->id;
		...
		// Authors
		if (is_array($doc->user_id)) {
			$user_id = $doc->user_id;
		} else {
			$user_id = array($doc->user_id);
		}
		
      // Title
      $tweets_content = format_title($doc->tweets_content, $url_display_basename);

      // Modified date
      $datetime = FALSE;
      if (isset($doc->created_on)) {
        $datetime = $doc->created_on;
      }
      elseif (isset($doc->created_on)) {
        $datetime = $doc->created_on;
      }
      ...
      ?>
      <li id="<?= $result_nr ?>">
        <div class="title"><a class="title" href="<?= $url_openfile ?>"><?= $tweets_content ?></a>
        </div>
        <div class="date"><?php 
        $myDateTime = new DateTime($datetime);
        echo $myDateTime->format('d/m/Y H:m');
        ...
        <div class="snippets">
          ...
          <?php if ($user_id): ?>
            <div class="author"><?= htmlspecialchars(implode(", ", $user_id)) ?></div>
          <?php endif; ?>
