...
function mask_query ( $query, $facets=array() ) {
	...
	$unmaskfacets = array('id','user_id','tweets_content','created_on');
...
function get_datevalues(&$results, $params, $downzoom) {
	...
	foreach ($results->facet_counts->facet_ranges->created_on->counts as $facet=>$count) {
		$newstart = $facet;
...
if (!$query) {
    $synonyms = false; // edited to false to prevent error 400
    $stemming = true;
}
...
function suggestionslink ($input){
	...
	for ($i = 0; $i < strlen($input); $i++) {
		if ($input[$i] == ":") {
			$input = substr($input, $i+1, strlen($input)-1);
		}
	}
	...
	return "Did you mean <a href='".$_SERVER["SCRIPT_NAME"]."?q=".htmlentities($suggestionText)."'>".$suggestionText."</a> ?";
...
if ($view != 'table' && $view != 'preview') {
	$additionalParameters['fl']='id,user_id,tweets_content,created_on';
...
if ($start_dt || $end_dt) {
	...
	$solrfilterquery .= ' +created_on:[ ' . $start_dt_solr . ' TO ' .$end_dt_solr. ']';
}
...
// (Solr default: score)
if ($sort == 'newest') {
	$additionalParameters['sort'] = "created_on desc";
} elseif ($sort == 'oldest') {
	$additionalParameters['sort'] = "created_on asc";
} elseif ($sort) {
...
// base facets fields
$arr_facets = array('created_on');


$additionalParameters['f.created_on.facet.mincount'] = 0;
$additionalParameters['facet.range'] = 'created_on';
...
if ($solrquery or $solrfilterquery) {
	...
	if ($solrquery != "*:*")
		$solrquery = "tweets_content:" . $solrquery;
	$results = false;
	try {
